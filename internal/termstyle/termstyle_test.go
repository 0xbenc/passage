...
func TestStripAndVisibleWidthCSISequences(t *testing.T) {
	cases := []struct {
		...
		input string
		want  string
		// width overrides the default len([]rune(want)) cell count for
		// cases where cell width differs from rune count (wide runes).
		width int
	}{
...
func TestStripAndVisibleWidthNonCSISequences(t *testing.T) {
	cases := []struct {
		...
		input string
		want  string
		// width overrides the default len([]rune(want)) cell count; the
		// "esc before control byte" case keeps a raw C0 control in the
		// visible text, which costs 0 cells (it costs 1 rune).
		width int
	}{
...
// TestVisibleWidthCellAccurate pins that width is measured in terminal cells,
// not runes: wide runes count 2, zero-width runes count 0, and SGR styling is
// ignored. This is what keeps padding/truncation aligned with the painter.
...
// TestBoxGlyphsStayWidthOne guards the locale landmine: the rounded-box and
// rule glyphs the shell draws must measure as a single cell so the chrome can
// never tear. The measure honors only RUNEWIDTH_EASTASIAN (not LANG), so this
// holds even when the test runs under a CJK locale.
...
func TestResolveThemeNoColorEnv(t *testing.T) {
	cases := []struct {
		...
		{"PASSAGE_NO_COLOR truthy", ThemeOptions{Env: []string{"PASSAGE_NO_COLOR=true"}, SkipDefaultFile: true}, true},
		{"PASSAGE_NO_COLOR falsy", ThemeOptions{Env: []string{"PASSAGE_NO_COLOR=0"}, SkipDefaultFile: true}, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestDefaultThemeUsesPaletteCodes(t *testing.T) {
	...
	assertStyle := func(role Role, want string) {
		t.Helper()
		got := theme.Style(role, "prod")
		if !strings.Contains(got, want) {
			t.Fatalf("%s style = %q, want %q", role, got, want)
		}
	}
	...
	for role := range theme.Codes {
		got := theme.Style(role, "prod")
		if strings.Contains(got, "38;2;") {
			t.Fatalf("%s style = %q, want no truecolor in default theme", role, got)
		}
	}
}
...
// passage (with role keys this binary does not know) must parse with
