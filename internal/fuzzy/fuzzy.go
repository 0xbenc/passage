// Package fuzzy is passage's thin adapter over termnav's shared fuzzy matcher.
// The fzf-style integer scoring that passage and ssherpa each carried a
// byte-identical copy of now lives once in github.com/0xbenc/termnav; this
// package re-exports it so the call sites (passstore ranking and highlight) are
// unchanged and a match scores identically across both apps. It still only ever
// sees non-secret metadata (paths/display names), never secret bytes.
package fuzzy
